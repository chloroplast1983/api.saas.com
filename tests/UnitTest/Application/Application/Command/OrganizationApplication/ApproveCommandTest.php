...
/**
 * Application\Command\OrganizationApplication/ApproveCommand.class.php 测试文件
 * @author chloroplast
 * @version 1.0.20160218
 */

class ApproveCommandTest extends GenericTestsDatabaseTestCase
{
     ...
     * 测试拒绝申请表单命令
     */
    public function testApproveCommand()
    {
        //查询状态为 !APPLICATION_VERIFIED 的旧数据,确认修改前状态
        $oldArray = Core::$_dbDriver->query('SELECT * FROM pcore_saas_application_organization WHERE status !='.APPLICATION_VERIFIED);
        ...
        //确认旧数据不是删除状态
        $this->assertNotEquals(APPLICATION_VERIFIED, $oldArray['status']);
        ...
        //初始化命令
        $command = Core::$_container->make('Application\Command\OrganizationApplication\ApproveCommand', ['organizationApplicationService'=>$organizationApplicationService]);
        //执行命令
        ...
        //期望对象状态已经修改
        $this->assertEquals(APPLICATION_VERIFIED, $organizationApplicationService->getApplication()->getStatus());
