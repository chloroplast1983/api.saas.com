<?php
/**
 * User\Repository\Query\User\UserRowCacheQuery.class.php 测试文件
 * @author chloroplast
...
class UserRowCacheQueryTest extends GenericTestCase
{
    ...
    {
        $this->stub = Core::$_container->get('User\Repository\User\Query\UserRowCacheQuery');
    }
    ...
    {
        $key = $this->getPrivateProperty('User\Repository\User\Query\UserRowCacheQuery', 'primaryKey');

        //判断primaryKey赋值设想一致
        $this->assertEquals('userId', $key->getValue($this->stub));
        //检查表是否有该字段
        $results = Core::$_dbDriver->query('SHOW COLUMNS FROM `'.$this->tablepre.'saas_user` LIKE \''.$key->getValue($this->stub).'\'');
    ...
    {
        $cacheLayer = $this->getPrivateProperty('User\Repository\User\Query\UserRowCacheQuery', 'cacheLayer');
    ...
    {
        $dbLayer = $this->getPrivateProperty('User\Repository\User\Query\UserRowCacheQuery', 'dbLayer');
