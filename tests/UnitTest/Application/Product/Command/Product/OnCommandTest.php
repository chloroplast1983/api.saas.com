...
/**
 * Product/Command/Product/OnCommand.class.php 测试文件
 * @author chloroplast
 * @version 1.0.20160218
 */

class OnCommandTest extends GenericTestsDatabaseTestCase
{
    ...
    public function testOnCommand()
    {
        ...
        //查询状态为 !PRODUCT_STATUS_ON_SALE 的旧数据,确认修改前状态
        $oldProductArray = Core::$_dbDriver->query('SELECT * FROM pcore_product WHERE status !='.PRODUCT_STATUS_ON_SALE.' LIMIT 1');
        ...
        //确认旧数据不是删除状态
        $this->assertNotEquals(PRODUCT_STATUS_ON_SALE, $oldProductArray['status']);
        ...
        //初始化命令
        $command = Core::$_container->make('Product\Command\Product\OnCommand', ['product'=>$this->product]);
        //执行命令
        ...
        //期望对象状态已经修改
        $this->assertEquals(PRODUCT_STATUS_ON_SALE, $this->product->getStatus());
