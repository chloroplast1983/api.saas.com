...
class DeleteCommandTest extends GenericTestsDatabaseTestCase
{
    ...
    public function testDeleteCommand()
    {
        ...
        //查询状态为 !PRODUCT_STATUS_DELETE 的旧数据,确认修改前状态
        $oldProductArray = Core::$_dbDriver->query('SELECT * FROM pcore_product WHERE status !='.PRODUCT_STATUS_DELETE.' LIMIT 1');
        ...
        //确认旧数据不是删除状态
        $this->assertNotEquals(PRODUCT_STATUS_DELETE, $oldProductArray['status']);
        ...
        //初始化命令
        $command = Core::$_container->make('Product\Command\Product\DeleteCommand', ['product'=>$this->product]);
        //执行命令
        ...
        //期望对象状态已经修改
        $this->assertEquals(PRODUCT_STATUS_DELETE, $this->product->getStatus());
