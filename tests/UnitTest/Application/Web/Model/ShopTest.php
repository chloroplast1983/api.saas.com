...
class ShopTest extends GenericTestCase
{
    ...
    {
        //测试初始化网店属主,saas用户对象
        $userParameter = $this->getPrivateProperty('Web\Model\Shop', 'user');
        $this->assertInstanceof('User\Model\User', $userParameter->getValue($this->stub));

        //测试初始化联系人电话
        $contactPeoplePhoneParameter = $this->getPrivateProperty('Web\Model\Shop', 'contactPeoplePhone');
        $this->assertEmpty($contactPeoplePhoneParameter->getValue($this->stub));

        //测试初始化商户名称
        $titleParameter = $this->getPrivateProperty('Web\Model\Shop', 'title');
        $this->assertEmpty($titleParameter->getValue($this->stub));

        //测试初始化联系人
        $contactPeopleParameter = $this->getPrivateProperty('Web\Model\Shop', 'contactPeople');
        $this->assertEmpty($contactPeopleParameter->getValue($this->stub));

        //测试初始化联系人qq
        $contactPeopleQQParameter = $this->getPrivateProperty('Web\Model\Shop', 'contactPeopleQQ');
        $this->assertEmpty($contactPeopleQQParameter->getValue($this->stub));

        //测试初始化店铺所在省
        $provinceParameter = $this->getPrivateProperty('Web\Model\Shop', 'province');
        $this->assertEmpty($provinceParameter->getValue($this->stub));

        //测试初始化店铺所在市
        $cityParameter = $this->getPrivateProperty('Web\Model\Shop', 'city');
        $this->assertEmpty($cityParameter->getValue($this->stub));

        //测试初始化店铺地址
        $addressParameter = $this->getPrivateProperty('Web\Model\Shop', 'address');
        $this->assertEmpty($addressParameter->getValue($this->stub));

        //测试初始化添加时间
        $createTimeParameter = $this->getPrivateProperty('Web\Model\Shop', 'createTime');
        $this->assertGreaterThan(0, $createTimeParameter->getValue($this->stub));

        //测试初始化状态变更时间
        $statusTimeParameter = $this->getPrivateProperty('Web\Model\Shop', 'statusTime');
        $this->assertGreaterThan(0, $statusTimeParameter->getValue($this->stub));

        //测试初始化状态
        $statusParameter = $this->getPrivateProperty('Web\Model\Shop', 'status');
        $this->assertEquals(SHOP_STATUS_NORMAL, $statusParameter->getValue($this->stub));

        //测试初始化商品分类
        $productTypeListParameter = $this->getPrivateProperty('Web\Model\Shop', 'productTypeList');
    ...
    //createTime 测试 -------------------------------------------------- start
    /**
     * 设置 Shop setCreateTime() 正确的传参类型,期望传值正确
     */
    public function testSetCreateTimeCorrectType()
    {
        $this->stub->setCreateTime(1461048737);
        $this->assertEquals(1461048737, $this->stub->getCreateTime());
    }

    /**
     * 设置 Shop setCreateTime() 错误的传参类型,期望期望抛出TypeError exception
     *
     * @expectedException TypeError
     */
    ...
    /**
     * 设置 Shop setCreateTime() 错误的传参类型.但是传参是数值,期望返回类型正确,值正确.
     */
    ...
    //statusTime 测试 -------------------------------------------------- start
    /**
     * 设置 Shop setStatusTime() 正确的传参类型,期望传值正确
     */
    public function testSetStatusTimeCorrectType()
    {
        $this->stub->setStatusTime(1461048737);
        $this->assertEquals(1461048737, $this->stub->getStatusTime());
    }

    /**
     * 设置 Shop setStatusTime() 错误的传参类型,期望期望抛出TypeError exception
     *
     * @expectedException TypeError
     */
    ...
    /**
     * 设置 Shop setStatusTime() 错误的传参类型.但是传参是数值,期望返回类型正确,值正确.
     */
    ...
    //productTypeList 测试 --------------------------------------------- start
    /**
     * 设置 Shop attachProductType() 正确的传参类型,期望传值正确
     */
    public function testAttachProductTypeCorrectType()
    {
        ...
        foreach ($counts as $count) {
            ...
            $this->stub->attachProductType($productType);
            $productTypeList[] = $productType;
        }
        $this->assertEquals($productTypeList, $this->stub->getProductTypeList());
    }

    /**
     * 设置 Shop attachProductType() 错误的传参类型,期望期望抛出TypeError exception
     *
     * @expectedException TypeError
     */
    public function testAttachProductTypeWrongType()
    {
        $this->stub->attachProductType('string');
    }
    //productTypeList 测试 ----------------------------------------------- end

    //updateTime 测试 -------------------------------------------------- start
    /**
     * 设置 Shop setUpdateTime() 正确的传参类型,期望传值正确
     */
    ...
    /**
     * 设置 Shop setUpdateTime() 错误的传参类型,期望期望抛出TypeError exception
     *
     * @expectedException TypeError
     */
    ...
    /**
     * 设置 Shop setUpdateTime() 错误的传参类型.但是传参是数值,期望返回类型正确,值正确.
     */
